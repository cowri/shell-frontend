...
//       _inputs[index] = Math.abs(+value)
//       setInputs(_inputs)
//       primeDeposit({ type: type, value: Math.abs(+value)})

//     }
//   }

//   const primeDepositNew = async (values) => {

//     const _errors = errors

//     for (const i = 0; i < values.length; i++) {

//       const numeraire = engine.assets[i].getNumeraireFromDisplay(values[i])
//       const balance = state.get('assets').get(i).get('numeraire')

//       if (numeraire.isGreaterThan(balance)) {
//         _errors
...
//         onChange={e => handleInput(e.target.value, ix)}
