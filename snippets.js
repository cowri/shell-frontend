...
const StartModal = ({
  ...
  const getMaxWithdraw = (ix) => {
    ...
    let toBurn = engine.calculateSingleWithdraw(shellIx, withdraw, ix)
    
    while (!toBurn || (toBurn && toBurn.isGreaterThan(ownedShells))) {
      
      withdraw = withdraw.times(new BigNumber('.99'))
      toBurn = engine.calculateSingleWithdraw(shellIx, withdraw, ix)
    ...
    while (true) {
        ...
        toBurn = engine.calculateSingleWithdraw(shellIx, test, ix)

        if (toBurn) toBurn = toBurn.abs()
        
        if (!toBurn || toBurn.isGreaterThan(ownedShells)) {

          upper = upper.minus(down)
          down = down.div(2)

        } else if (toBurn.isLessThanOrEqualTo(ownedShells)) {

          withdraw = test

          if (toBurn.isEqualTo(ownedShells)) break
          if (pToBurn.minus(toBurn).abs().lte('.00000001')) break

        } 

    }
    
    const decimals = new BigNumber(10 ** (engine.shells[shellIx].assets[ix].decimals - 2))
    const one = new BigNumber('1').div(decimals)
  ...
  const withdrawMaxNoSlip = (ix) => {
    ...
    let test = withdraw
    let pToBurn = engine.calculateSingleWithdraw(shellIx, withdraw, ix).abs()
    let toBurn
    let proximity
    
    const sweetspot = '.0000000001'
    const sliptarget = engine.shells[shellIx].epsilon.times(-1)
    ...
    let shellsChange = pToBurn.div(shellsTotal)
    let slippage = new BigNumber(1).minus(shellsChange.div(liquidityChange))
    
    let multiplier = slippage.gte(sliptarget)
      ? new BigNumber('1.025')
      : new BigNumber('.975')

    let adjuster = new BigNumber('.0125')
    
    let iterate = slippage.plus(sliptarget).abs().gte(sweetspot)
    
    console.log(" _-_-_-_-_-_-_-_-_-_-_-_-_ ")
    console.log("slippage", slippage.toString())
    console.log("sliptarget", sliptarget.toString())
    console.log("combined", slippage.minus(sliptarget).toString())

    while (iterate) {

      test = withdraw.times(multiplier)
      
      toBurn = engine.calculateSingleWithdraw(shellIx, test, ix).abs()

      liquidityChange = test.div(liqTotal)
      shellsChange = pToBurn.div(shellsTotal)
      slippage = new BigNumber(1).minus(shellsChange.div(liquidityChange))

      proximity = toBurn.minus(pToBurn).abs()

      console.log("_-_-_-_-_-_-_-_-_-_-_-_")
      // console.log("liq removed", test.toFixed(10))
      // console.log("liq total", liqTotal.toString())
      // console.log("liquidity change", liquidityChange.toString())
      console.log("shells to burn", toBurn.toString())
      console.log("p shells to burn", pToBurn.toString())
      console.log("proximity", proximity.toString())
      // console.log("shells total", shellsTotal.toString())
      // console.log("shells change", shellsChange.toString())
      console.log("slippage ", slippage.toString())
      
      if (proximity.lte(sweetspot)) {
        withdraw = test
        break
      } else {

        const slipdiff = slippage.minus(sliptarget).abs()
        
        console.log("slippdiff", slipdiff.toString())
        
        if (slipdiff.gte(sweetspot)) {

          pToBurn = toBurn
          withdraw = test

        } else if (slippage.lte(sliptarget)) {

          multiplier = multiplier.plus(adjuster)
          adjuster = adjuster.div(2)

        } else {
          
          multiplier = multiplier.minus(adjuster)
          adjuster = adjuster.div(2)
  ...
  const primeWithdraw = async () => {
    ...
    const toBurn = await engine.shells[shellIx].viewSelectiveWithdraw(addresses, amounts)
    const calculatedShellsToBurn = engine.calculateWithdraw(shellIx, amts)
    
    if (toBurn === false || toBurn.toString() == REVERTED) {
      
      setError(SAFETY_CHECK)
      setFeeTip(null)
      return

    } else if (toBurn.isGreaterThan(state.getIn(['shells', shellIx, 'shell', 'shellsOwned', 'numeraire']))) {
    ...
    const shellsTotal = state.getIn(['shells', shellIx, 'shell', 'shellsTotal', 'numeraire'])
    const shellsChange = toBurn.div(shellsTotal)

    const slippage = new BigNumber(1).minus(shellsChange.div(liquidityChange))

    console.log("<>!<!><!><!>")
    
    console.log("liq remove", totalWithdraw.toString())
    console.log("liq total", liqTotal.toString())
    ...
    console.log("slippage", slippage.toString())
    console.log("epsilon", engine.shells[shellIx].epsilon.toString())
    
    const fee = toBurn.times(slippage)
    ...
    const shells = (
      <div>
        ...
        <span style={{position: 'relative', paddingLeft: '16.5px', paddingRight: '4px' }}> 
          ...
          { ' ' + engine.shells[shellIx].getDisplayFromNumeraire(toBurn) }
        </span>
        { slippageMessage }
      </div>
    )
